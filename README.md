...
5. **recipient**: Wallet address on the destination chain we want to transfer our tokens to.

6. **amount**: A uint256 variable specifying the amount of tokens we want to transfer to the recipient on the destination chain

The function burns the amount of tokens specified in the variable "amount" from the user's account and creates a cross-chain communication request to the destination chain. The payload for the request is generated by ABI encoding "amount" and "recipient".

The function uses the requestToDest function of the Gateway Contract to generate the cross-chain communication request to the destination chain. The function also uses the ourContractOnChains mapping to retrieve the address of the CrossChainERC20 contract on the destination chain. For details on how the requestToDest function works ,refer our [docs](https://devnet-docs.routerprotocol.com/crosstalk/understanding-crosstalk/requestToDest)
